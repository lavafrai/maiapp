...
- **Android:** все клиенты используют Ktor Android / HttpsURLConnection со штатным
  ...
  Конфигурация доменов действует на всё приложение: CIO здесь несовместим,
  поскольку вызывает `checkServerTrusted` без имени сервера. Поэтому общий
  клиент и `AndroidPlatform.ktorEngine()` используют одну платформенную фабрику,
  а зависимость CIO исключена из Android, включая автоматический выбор движка
  в `HttpClient()`. JVM использует свои зависимости независимо.
...
Android дополнительно проверяет общий клиент и автоматический выбор движка,
а также то, что цепочка МАИ не становится доверенной для другого поддомена МАИ.
